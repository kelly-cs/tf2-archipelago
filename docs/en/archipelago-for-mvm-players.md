...
| `Weapon Buff: Reserve Shooter — ...` and other curated weapon/effect combinations | Adds the named passive effect whenever that weapon or a functional reskin is equipped |
...
By default, weapon slots, class items and mission tickets are progression items
and weapon buffs are useful items here. An
...
`Cash Bundle` is the literal filler classification here. Cash rewards are off
by default, so weapon buffs fill every spare local check. The Rewards options
can enable cash and can independently make tickets, classes, slots or buffs
useful or required for progression.
