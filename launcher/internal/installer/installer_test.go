...
func TestCommunityDownloadManifestIncludesIconsAndAliasedTextures(t *testing.T) {
	...
	write(filepath.Join(populationDir, "mvm_legerdemain_a6e_adv_test.pop"),
	...
	body, err := os.ReadFile(filepath.Join(modDir, "addons", "sourcemod", "data", "tf2_archipelago", "downloads", "mvm_legerdemain_a6e.txt"))
	if err != nil {
		t.Fatal(err)
	}
