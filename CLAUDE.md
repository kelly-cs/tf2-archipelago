...
An Archipelago randomiser for TF2 Mann vs Machine: an apworld, a bridge, a
SourceMod plugin, and a Windows launcher that hides the dedicated-server setup
behind one button.

- `apworld/` — the Archipelago world (Python). Logic lives in `rules.py`.
- `bridge/` — Go, connects the game server to the multiworld.
- `plugin/` — the SourceMod side.
- `launcher/` — the Windows .exe players actually run.
- `gamedata/` — missions, waves, item and location tables.
- `CONTEXT.md` — **read this first.** Archipelago and TF2 use some of the same
  words for different things; this file defines both vocabularies.

## Build and test

Use the Makefile targets, not raw commands. `make help` lists them.

```bash
make test          # Go tests
make lint          # golangci-lint
make apworld-test  # apworld tests
make apworld-lint
make seed          # generate a seed
make up / down / logs / rcon   # run the server locally
```

`CHANGELOG.md` is player-facing and `.github/workflows/release.yml` reads the
section matching the tag into the release notes, so it is the only place to
write them. Do not put developer notes there.
...
- Prefix `apw`. Priorities: P0 crashes and data loss, P1 bugs that cost a
