...
func TestCommunityManifestLoadsMapsMissionsAndUpgrades(t *testing.T) {
	content, err := loadCommunity([]byte(`{
		...
		"missions": [{
			...
			"has_tank": true,
			"has_giant": true,
			"upgrades_file": "scripts/items/mvm_upgrades_example.txt",
			"requires": "rafmod"
		}]
	}`))
	if err != nil {
	...
	if got := content.UpgradeFiles[101]; got != "scripts/items/mvm_upgrades_example.txt" {
		t.Fatalf("upgrades file = %q", got)
	}
	if got := content.Requirements[101]; got != "rafmod" {
...
func TestRafModMissionsAreTombstonesNotPlayableChoices(t *testing.T) {
	for _, mission := range Missions {
		if mission.PopFile == "mvm_kelly_rc1b_adv_mobocracy" && IsPlayableMission(mission.ID) {
			t.Fatal("Mobocracy is playable without a current RafMod")
		}
	}
...
func TestMissionsWithoutNavigationAreNotPlayableChoices(t *testing.T) {
	for _, mission := range Missions {
		if (mission.PopFile == "mvm_bogland_rc12_adv_swamp_fever" || mission.PopFile == "mvm_cyberia_rc6a_adv_silver_snow") && IsPlayableMission(mission.ID) {
			t.Fatalf("%s is playable without a navigation mesh", mission.PopFile)
		}
	}
}
