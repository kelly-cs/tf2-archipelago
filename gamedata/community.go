...
type loadedCommunity struct {
	Maps         []Map
	Missions     []Mission
	UpgradeFiles map[MissionID]string
...
func loadCommunity(body []byte) (loadedCommunity, error) {
	...
	content := loadedCommunity{
		...
		UpgradeFiles: make(map[MissionID]string, len(file.Missions)),
	...
	for _, entry := range file.Missions {
		...
		if entry.Requires != "" && entry.Requires != "rafmod" && entry.Requires != "no_nav" {
		...
		content.UpgradeFiles[entry.ID] = entry.UpgradesFile
...
func ValidateCommunitySources(sources ...string) error {
	...
	for _, m := range communityMissions {
		...
		if upgrades := MissionUpgradesFile(m.ID); upgrades != "" {
			required[upgrades] = "upgrade table " + upgrades
		}
	}
...
func IsCommunityMission(id MissionID) bool {
	_, ok := communityContent.UpgradeFiles[id]
	return ok
}

// IsPlayableMission reports whether this build can safely put a mission in a
// seed. Valve and stock-syntax community missions are playable everywhere;
// RafMod tombstones remain in the ID table but are never offered or drawn.
...
// MissionUpgradesFile returns the game-relative custom upgrade file selected
// for a mission. Blank means scripts/items/mvm_upgrades.txt.
